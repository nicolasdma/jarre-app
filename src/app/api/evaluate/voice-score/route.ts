 ...
 * Response: Same shape as /api/evaluate/submit
 *
 * Flow:
...
const VOICE_EVAL_PROMPT_VERSION = 'voice-v1.0.0';
...
export const POST = withAuth(async (request, { supabase, user }) => {
  try {
    ...
    const questions = concepts.map((concept: { id: string; name: string }, i: number) => {
      // Find the main evaluator question for this concept from the transcript
      ...
      return {
        ...
        type: 'explanation' as const, // Voice evals are fundamentally explanation-based
