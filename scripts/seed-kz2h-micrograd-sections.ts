/**
 * Seed script for kz2h-micrograd sections.
 *
 * Reads sections from scripts/output/kz2h-micrograd-resegmented.json
 * and inserts them into resource_sections table.
 *
 * Usage: npx tsx scripts/seed-kz2h-micrograd-sections.ts
 */
...
const VALID_CONCEPTS = ['neural-network-fundamentals', 'backpropagation-training'];
...
async function main() {
  // 1. Read JSON
  const jsonPath = resolve(__dirname, 'output/kz2h-micrograd-resegmented.json');
  ...
  if (existingIds.length > 0) {
    // question_bank FK is NOT CASCADE — must delete manually
