/**
 * Seed script for "The Tail at Scale" paper sections.
 *
 * Reads sections from scripts/output/tail-scale-resegmented.json
 * and inserts them into resource_sections table.
 *
 * Usage: npx tsx scripts/seed-tail-scale-sections.ts
 */
...
const VALID_CONCEPTS = ['tail-latency', 'hedged-requests', 'fan-out-latency'];
...
async function main() {
  // 1. Read JSON
  const jsonPath = resolve(__dirname, 'output/tail-scale-resegmented.json');
  ...
  if (existingIds.length > 0) {
    // Delete dependent rows: inline_quizzes (CASCADE handles this),
    // but question_bank FK is NOT CASCADE — must delete manually
